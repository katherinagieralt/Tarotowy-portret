...
export function Footer() {
  ...
  return (
    <footer className="w-full bg-[#FDFBF7] dark:bg-[#0A0710] pt-20 pb-12 sm:pb-8 mt-auto border-t border-black/5 dark:border-white/5 transition-colors duration-500 relative z-10 overflow-hidden">
      {/* Ozdobny gradient w tle */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-3/4 h-32 bg-amber-500/5 dark:bg-indigo-500/10 blur-[100px] rounded-full pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 lg:gap-8 items-center md:items-start mb-16">
          ...
          <div className="md:col-span-5 flex flex-col items-center md:items-start text-center md:text-left">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed max-w-sm font-light">
              {isEnglish 
                ? "Discover the secrets of your soul through precise archetype analysis. Your personal guide on the path to self-awareness and deep understanding of your own potential."
          ...
          <div className="md:col-span-3 flex flex-col items-center md:items-start">
            <h2 className="text-slate-900 dark:text-white font-serif font-bold text-lg mb-6 tracking-wide">
              {isEnglish ? "Explore" : "Odkrywaj"}
            </h2>
            <nav className="flex flex-col gap-4 text-sm font-medium text-slate-600 dark:text-slate-300">
              <Link href={isEnglish ? "/" : "/pl"} className="hover:text-amber-600 dark:hover:text-amber-400 transition-colors flex items-center gap-2 group">
                <span className="w-0 h-[1px] bg-amber-500 transition-all duration-300 group-hover:w-4"></span>
                {isEnglish ? "Calculator" : "Kalkulator"}
              </Link>
              <Link href={isEnglish ? "/arkany" : "/pl/arkany"} className="hover:text-amber-600 dark:hover:text-amber-400 transition-colors flex items-center gap-2 group">
                <span className="w-0 h-[1px] bg-amber-500 transition-all duration-300 group-hover:w-4"></span>
                {isEnglish ? "Major Arcana" : "Wielkie Arkana"}
              </Link>
              <Link href={isEnglish ? "/pozycje-portretu" : "/pl/pozycje-portretu"} className="hover:text-amber-600 dark:hover:text-amber-400 transition-colors flex items-center gap-2 group">
                <span className="w-0 h-[1px] bg-amber-500 transition-all duration-300 group-hover:w-4"></span>
                {isEnglish ? "Portrait Positions" : "Pozycje portretu"}
              </Link>
              <Link href={isEnglish ? "/kontakt" : "/pl/kontakt"} className="hover:text-amber-600 dark:hover:text-amber-400 transition-colors flex items-center gap-2 group">
                <span className="w-0 h-[1px] bg-amber-500 transition-all duration-300 group-hover:w-4"></span>
                {isEnglish ? "About Us & Contact" : "O nas & Kontakt"}
              </Link>
            </nav>
          </div>
          ...
          <div className="md:col-span-4 flex flex-col items-center md:items-start">
            ...
            <div className="flex gap-4">
              ...
              {/* TikTok */}
              <SocialIcon href="https://www.tiktok.com/@getarcheya" ariaLabel="TikTok">
                <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5" aria-hidden="true">
                  <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z"/>
                </svg>
              </SocialIcon>
              {/* Pinterest */}
              <SocialIcon href="https://pin.it/4yw3aie4O" ariaLabel="Pinterest">
                <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5" aria-hidden="true">
        ...
        <div className="pt-8 border-t border-black/5 dark:border-white/5 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-slate-500 dark:text-slate-400 text-xs tracking-wider">
            &copy; {currentYear} ARCHEYA. {isEnglish ? "All rights reserved." : "Wszystkie prawa zastrzeżone."}
          </p>
          <div className="flex gap-6 text-xs text-slate-500 dark:text-slate-400">
            <Link href={isEnglish ? "/polityka-prywatnosci" : "/pl/polityka-prywatnosci"} className="hover:text-amber-500 transition-colors">
              {isEnglish ? "Privacy Policy" : "Polityka prywatności"}
            </Link>
            <Link href={isEnglish ? "/regulamin" : "/pl/regulamin"} className="hover:text-amber-500 transition-colors">
              {isEnglish ? "Terms of Service" : "Regulamin"}
            </Link>
          </div>
        </div>
      </div>
