...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `/pl/arkana/${resolvedParams.slug}`,
      languages: {
    ...
    <main className="min-h-screen bg-[#F9F6EE] dark:bg-[#0A0710] py-20 px-4 transition-colors duration-500">
      <div className="max-w-5xl mx-auto">
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              '@type': 'Article',
              headline: `${String(post.frontmatter.title)} - Tarotowy Portret`,
              description: String(post.frontmatter.description),
              ...
              mainEntityOfPage: {
                '@type': 'WebPage',
                '@id': `https://getarcheya.com/pl/arkana/${resolvedParams.slug}`
              }
            })
          }}
        />
            ...
            __html: JSON.stringify({
              ...
              itemListElement: [
                ...
                },
                {
                  ...
                  item: `https://getarcheya.com/pl/arkana/${resolvedParams.slug}`
                }
              ]
            })
          }}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3">
                {Object.entries(individualPositionMeanings).map(([key, pos]) => (
                  <Link 
                    key={key} 
                    href={`/pl/znaczenie/${generatePositionSlug(post.slug, false, key)}`}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3">
                {Object.entries(partnerPositionMeanings).map(([key, pos]) => (
                  <Link 
                    key={key} 
                    href={`/pl/znaczenie/${generatePositionSlug(post.slug, true, key)}`}
                    className="group flex items-center px-3 py-1.5 sm:px-4 sm:py-2 text-sm sm:text-base rounded-lg bg-white/50 dark:bg-slate-900/60 border border-black/5 dark:border-white/5 hover:border-amber-500/40 hover:bg-amber-50 dark:hover:bg-[#1A1625] text-slate-700 dark:text-slate-300 transition-all duration-300 shadow-sm hover:shadow-md"
                  >
                    <span className="text-amber-600 dark:text-amber-500 mr-2 font-bold text-xs opacity-70 group-hover:opacity-100">
                      {key.replace('p', '').padStart(2, '0')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-black/5 dark:border-white/5 pt-12">
          {prevPost && (
            <Link href={`/pl/arkana/${prevPost.slug}`} className="group flex flex-col items-start bg-white/40 dark:bg-slate-900/40 hover:bg-white/80 dark:hover:bg-slate-800/80 p-6 rounded-2xl border border-transparent hover:border-amber-500/30 transition-all duration-300">
          ...
          {nextPost && (
            <Link href={`/pl/arkana/${nextPost.slug}`} className="group flex flex-col items-end text-right bg-white/40 dark:bg-slate-900/40 hover:bg-white/80 dark:hover:bg-slate-800/80 p-6 rounded-2xl border border-transparent hover:border-amber-500/30 transition-all duration-300">
              <span className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wider flex items-center gap-2">
